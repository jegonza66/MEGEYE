function fun_check_behavior(isub,session_path,runpath)
    if(~exist([session_path.rawet,'/out']))
        mkdir([session_path.rawet,'/out'])
    end

    Nsubj_sessions=size(session_path.behavfilenames{isub},1); %loop over sessions
    for isess=1:Nsubj_sessions
        session_name=session_path.behavfilenames{isub}{isess};
        
        beh.global_beh_file = fullfile(session_path.rawet,[session_name '.mat']);
        beh.trials_dir      = fullfile(session_path.rawet,[session_name '_Trials']);
        listfile_raw        = dir(fullfile(beh.trials_dir,[session_name '_Trial*']))  ;
        
        % Load SingleTrial data
        cd(beh.trials_dir)
            ...
            for ikk=1:length(listfile_raw)
                ...
                % dbstop in fun_check_behavior at 29 if itrial==31
                clear SingleTrial;
        ...
        buttons_CFS_file=fullfile(session_path.rawet,['buttons_CFS_' session_name(1:3) '.mat']);
        ...
        legend boxoff
        outfile=fullfile(session_path.rawet,'out',['buttons_time' session_name]); print(gcf,'-dpng',outfile);
        ...
        y = [0 1 1 0];
        fill(x,y,cfg.colortasks{3},'FaceAlpha',alpha_value);
        ylabel(['Norm Acc Correct, Session: ' session_name],'Interpreter','none');
        xlabel(['Trial']);
        outfile=fullfile(session_path.rawet,'out',['cdf_correct_' session_name]); print(gcf,'-dpng',outfile);
        %%         %% [ind_VS] = arrayfun(@(x)find(strmatch(x,'VS')==1),trial_type,'uniformoutput',false)
        figure;clf;
        ...
        y = [0 1 1 0];
        fill(x,y,cfg.colortasks{3},'FaceAlpha',alpha_value);
        ylabel(['Performance, Session: ' session_name],'Interpreter','none');
        xlabel(['Trial']);
        outfile=fullfile(session_path.rawet,'out',['perfo_trial_' session_name]); print(gcf,'-dpng',outfile);  
...
%         isME_trial = cell2mat(cellfun(@(x) strcmp(x,'ME'), trial_type, 'UniformOutput',false));
        ...
        legend(cfg.tasks)
        legend boxoff
        outfile=fullfile(session_path.rawet,'out',['performance_' session_name]); print(gcf,'-dpng',outfile);
        ...
        legend(cfg.tasks)
        legend boxoff
        outfile=fullfile(session_path.rawet,'out',['perf_F_OBJ_' session_name]); print(gcf,'-dpng',outfile);
    end
end
